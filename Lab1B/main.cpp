...
void adaptiveLoop(Game& game, float& lastTime, float updateTarget = 0)
{
	...
	//      constant framerate.
	auto afterThisLoop = game.getElapsed().asSeconds();
	sf::sleep(sf::seconds(updateTarget - (afterThisLoop - current)));

	// I.C: Calculate the current frame rate and set it to the game.
	auto frameTime = game.getElapsed().asSeconds() - current;
	auto Fps = static_cast<int>(1.0f / frameTime);

	// I.D: Call the function game.setFPS(int) to inform the game about the current FPS. Print this value to console as well.
	game.setFPS(Fps);
	std::cout << "Frametime: " << frameTime << "\tFPS: " << Fps << std::endl;
