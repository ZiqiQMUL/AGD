...
#include "../graphics/AnimDirectional.h"
#include <fstream>
#include <stdexcept>
...
bool SpriteSheet::isCurrentAnimation(const std::string& animationName) const
{
	if (curAnimation != nullptr)
	{
		return curAnimation->getName() == animationName;
	}
	return false;
}

bool SpriteSheet::isAnimationPlaying() const
{
	if (curAnimation != nullptr)
	{
		return curAnimation->isPlaying();
	}
	return false;
}

bool SpriteSheet::isAnimationInAction() const
{
	if (curAnimation != nullptr)
	{
		return curAnimation->isInAction();
	}
	return false;
}
